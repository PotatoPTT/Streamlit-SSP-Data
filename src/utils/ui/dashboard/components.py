...
def render_data_table_section(dados):
    ...
    tabela_completa = processar_tabela_detalhada(dados['df_dados'], dados['df_anterior'])
...
def render_maps_section(year_filter):
    ...
    mapas_existem, crimes_mapas, mapas_ano_path = verificar_mapas_disponiveis(year_filter)
    
    if not mapas_existem:
        # Gerar mapas sob demanda
        st.info(f"🗺️ Mapas não encontrados para {year_filter}. Iniciando geração...")
        ...
        with st.spinner(f"Gerando mapas para o ano {year_filter}..."):
            ...
            try:
                ...
                logger.error(f"[MAPS] Erro ao gerar mapas para {year_filter}: {e}")
