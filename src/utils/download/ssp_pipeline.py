    ...
    def run(self):
        ...
        if not os.path.exists(loc_file):
            ...
            try:
                ...
                unique_codes = codes_df['Nome_Municipio'].drop_duplicates().sort_values().reset_index(drop=True)
                if 'Nome_Municipio' not in loc_df.columns or 'latitude' not in loc_df.columns or 'longitude' not in loc_df.columns:
                    ...
                    merged = pd.merge(unique_codes.to_frame(), loc_df, on='Nome_Municipio', how='left')
                    missing = merged[merged['latitude'].isnull() | merged['longitude'].isnull()]
                    if len(loc_df) != len(unique_codes) or not missing.empty:
                        needs_location_update = True
            except Exception as e:
                logging.error(f"Erro ao verificar integridade de cities_location.csv: {e}")
                needs_location_update = True

        # Verificar e gerar arquivo de localização de cidades se necessário
        if needs_location_update:
            logging.warning(f"Arquivo de localização ausente ou incompleto. Gerando paralelamente aos downloads...")
            with ThreadPoolExecutor(max_workers=2) as executor:
                f_loc = executor.submit(self.downloader.generate_location_file, output_location_path=self.location_csv)
